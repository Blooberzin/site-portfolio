import { signSlide } from "@/lib/auth";

export const runtime = "nodejs";
export const maxDuration = 30;

async function signMedia(urls: string[]) {
  const exp = String(Math.floor(Date.now() / 1000) + 60 * 60 * 24);
  return Promise.all(urls.map(async (raw) => {
    const url = new URL(raw);
    const index = (url.searchParams.get("index") || "1").slice(0, 2);
    const title = (url.searchParams.get("title") || "Teste IA").slice(0, 120);
    const body = (url.searchParams.get("body") || "").slice(0, 260);
    const sig = await signSlide({ index, title, body, exp });
...
export async function POST(request: Request) {
  try {
    ...
    let media = Array.isArray(body.media) ? body.media.filter(Boolean) : [];
    if (network === "instagram" && media.length === 0) {
      return Response.json({ error: "Instagram exige mídia." }, { status: 400 });
    }
    ...
    if (network === "instagram") media = await signMedia(media);

    const payload = {
      publicationDate: { dateTime: body.dateTime, timezone },
      text: body.text,
      providers: [{ network }],
      autoPublish: true,
      draft: false,
